...
const tagNameAttrsToText = ({name, attribs}, deep = 0) => {
    if (deep) {
        ...
        if (lenAttribs >= WRAP_ATTRIBUTES_COUNT) {
            return [
                `${getSpace(deep)}<${name}`,
                ...attribsToPlainText(attribs).map(v => `${getSpace(deep + 1)}${v}`),
                `${getSpace(deep)}>`
            ].join('\n');
        }
    }
    return `${getSpace(deep)}<${[name, ...attribsToPlainText(attribs)].filter(v => v).join(' ')}>`;
};
            ...
            if (err) {
                ...
                dom.filter(v => v.name === 'script').forEach((v, i) => {
                    if (v.children.length) {
                        ...
                        promiseList.push(
                            formatText(strScript, 'babylon', {name: 'script', attribs: {}})
                        );
                    }
                });
